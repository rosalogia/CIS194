...
parseBNF = parseBNF
    where
        parseBnf = nonTerminal "bnf" $
            pure (,)    <*> parseProduction
                        <*> many (pure (,) <*> parseProduction)

        parseProduction = nonTerminal "production" $
        ...
        parseRhs = recNonTerminal "rhs" $ \rhs ->
            parseChoice

        parseChoice = nonTerminal "choice" $
            pure (,)    <*> parseSequence
                        <*> many (pure (,)  <* spaces
                                            <* char ','
                                            <* spaces
        ...
        parseSequence = nonTerminal "sequence" $
            pure (,)    <*> parseAtom
                        <*> many (pure (,)  <* spaces
                                            <* char ','
                                            <* spaces
        ...
        parseAtom = nonTerminal "atom" $
            pure (,)    <*> parseTerminal
                        `orElse` parseNonTerminal
                        `orElse` parseOption
                        `orElse` parseRepetition
                        `orElse` parseGroup

        parseGroup = nonTerminal "group" $
            pure (,)    <* char '('
                        <* spaces
                        <*> parseRhs
                        <* spaces
        ...
        parseRepetition = nonTerminal "repetition" $
            pure (,)    <* char '{'
                        <* spaces
                        <*> parseRhs
                        <* spaces
        ...
        parseOption = nonTerminal "option" $
            pure (,)    <* char '['
                        <* spaces
                        <*> parseRhs
                        <* spaces
        ...
        parseNonTerminal = nonTerminal "non-terminal" $
            pure (,)    <*> parseIdentifier
                        <* spaces

        parseTerminal = nonTerminal "terminal" $
            pure (,)    <* char '\''
                        <*> parseQuotedChar
                        <* char '\''
                        <* spaces

        parseQuotedChar = nonTerminal "parseQuotedChar" $
            pure (,)    <*> notQuoteOrBackslash
                        `orElse` char '\\' <*> char '\\'
                        `orElse` char '\\' <*> char '\''

        parseSpaces = nonTerminal "spaces" $
            pure (,)    many (pure (,) <* char ' ' `orElse` char '\n')

        parseIdentifier = nonTerminal "identifier" $
            pure (,)    <*> letter
                        <*> many (pure (,)  <*> letter
                                            `orElse` digit
                                            `orElse` (char '-'))
...
runGrammar main (G (prods, NonTerminal nt)) | main == nt = prods
runGrammar main (G (prods, rhs)) = prods ++ [(main, rhs)]

ppGrammar :: String -> Grammar a -> String
ppGrammar main g = ppBNF $ runGrammar main g
